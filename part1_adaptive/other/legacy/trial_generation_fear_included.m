...
for i = 1:110
    ...
    for j = 1:4
        ...
        dist_ratings([i-1]*4+j).PercentageCorrect = percentage_correct;
        dist_ratings([i-1]*4+j).PercentageWrong = percentage_false;
...
for i = 1:5
    hold on
    plot(cell_trials{i}.PercentageCorrect,'LineWidth',2)
end
legend(emo_labels),xlabel('Trials'),ylabel('Percentage Correct'),title('Percentage of correct answer')
hold off
subplot(1,3,2)
for i = 1:5
    hold on
    plot(sort(cell_trials{i}.Distance,'descend'),'LineWidth',2)
end
...
for i = 1:5
    hold on
    plot(sort(cell_trials{i}.Ttest,'descend'),'LineWidth',2)
end
...
%writetable(trials,'trials_fear.csv')
