...
%% Figure 1a Boxplot of emotion item agreement
colorsfig1 = [
...
% Scatter points with jitter for each pair
for k = 1:20
...
meanVals = grpstats(rasch_mirt{:,2}, trial_info{:,2}, 'median');
[~, sortIdx] = sort(meanVals, 'ascend');
% Gather data
all_agreements = [];
all_group_ids = [];
...
for i = sortIdx'
    ...
    jitter = (rand(size(this_vals)) - 0.5) * 0.3;
    scatter(this_vals, i + jitter, ...
        60, 'MarkerFaceColor', colorsfig1(i,:), ...
        'MarkerFaceAlpha', 0.4, 'MarkerEdgeColor', 'none');
end
% Plot horizontal boxplot
boxplot(all_agreements, categorical(all_group_ids), ...
    'Orientation', 'horizontal', ...
    'Widths', 0.5, ...
    'Symbol', '','GroupOrder',cellstr(string(sortIdx)));
% Style axes and labels
xlabel('Item Agreement', 'FontSize', 22)
set(gca, 'FontSize', 20, 'LineWidth', 2)  % Tick font size (including y-axis/emotion labels)
yticklabels(emoNames(sortIdx));         % Set the y-axis labels
set(gca, 'FontSize', 22);
ylim([0.5, 5.5])
%title('Item Agreement by Emotion (Horizontal Boxplots)', 'FontSize', 24)
grid on
box on
% Recolor and restyle boxplot elements (reverse order fix!)
boxHandles     = flipud(findobj(gca, 'Tag', 'Box'));
whiskerHandles = flipud(findobj(gca, 'Tag', 'Whisker'));
medianHandles  = flipud(findobj(gca, 'Tag', 'Median'));
capHandles     = flipud(findobj(gca, 'Tag', 'Cap'));
for i = 1:5
    ci = colorsfig1(i,:);
    if i <= length(boxHandles)
        set(boxHandles(i), 'Color', ci, 'LineWidth', 2.5)
    end
    if i <= length(medianHandles)
        set(medianHandles(i), 'Color', ci, 'LineWidth', 2.5)
    end
    if i*2 <= length(whiskerHandles)
        set(whiskerHandles(i*2-1), 'Color', ci, 'LineWidth', 3.5)
        set(whiskerHandles(i*2),   'Color', ci, 'LineWidth', 3.5)
    end
    if i*2 <= length(capHandles)
        set(capHandles(i*2-1), 'Color', ci, 'LineWidth', 3.5)  % ← thicker caps
        set(capHandles(i*2),   'Color', ci, 'LineWidth', 3.5)
    end
end

% Find all lines and set linewidth and linestyle for those near boxplot y positions
allLines = findall(gca, 'Type', 'Line');

yVals = 1:5; % your 5 emotions (adjust if needed)
yTol = 0.1;  % tolerance for y-position filtering

for k = 1:length(allLines)
    yData = get(allLines(k), 'YData');
    % If line is mostly horizontal around one of the boxplot y-levels, treat as whisker/cap
    if all(abs(yData - round(yData)) < yTol)
        allLines(k).LineWidth = 1;
        allLines(k).LineStyle = '--';
    end
end
hold off
...
tss_sc = tss(theta_range<2.4 & theta_range>-2.4);%scatter plot axis for figure 3
...
title('Item Difficulty','FontSize',24);
text(-0.2,1,'a','Units','normalized','FontSize',24,'FontWeight','bold')
%xline(mean(rasch_mirt{:,2}),'-',{'Mean'},'LineWidth',3);
box on
grid on
hold off
...
title('Participant Ability','FontSize',24);
text(-0.16,1,'b','Units','normalized','FontSize',24,'FontWeight','bold')
...
set(gca,'FontSize',20,'LineWidth',2)
title('Test True Score','FontSize',24);
...
text(-0.2,1,'c','Units','normalized','FontSize',24,'FontWeight','bold')
box on
grid on
...
xlabel('Ability Level (θ)');
set(gca,'XTick',1:20:length(theta_range),'XTickLabel',theta_low:theta_high)
title('Test Information & Standard Error','FontSize',24)
%legend('Information','Standard Error','Location','northeast','FontSize',14)
lgd = legend('Information','Standard Error');
lgd.FontSize = 14;
...
xlim([1,length(theta_range)])
text(-0.16,1,'d','Units','normalized','FontSize',24,'FontWeight','bold')
box on
grid on
hold off
%linkaxes([ax3, ax4], 'y');
...
figure
t = tiledlayout(2,2,'Padding','loose', ...
    'TileSpacing','loose');
ax1 = nexttile([1 2]);
hold(ax1,'on')
scatter(rasch_mirt{:,2},difficulty_item,80,scatter_color,'filled')
...
ylabel('Item Agreement')
title('Item Agreement and Item Difficulty','FontSize',24)
text(-0.1,1,'a','Units','normalized','FontSize',24,'FontWeight','bold')
box on
grid on
hold off
...
ylabel('Accuracy');
xlabel('Item difficulty (Model)');
title('Accuracy and Item Difficulty','FontSize',24)
%title({['Item Classification Accuracy'], ['r=' num2str(round(corr(accuracy_i',...
%    rasch_mirt{:,2}),2))]},'FontSize',28)
text(-0.22,1,'b','Units','normalized','FontSize',24,'FontWeight','bold')
box on
grid on
hold off
...
title('Accuracy and Participant Ability','FontSize',24)
%title({['Participant Classification Accuracy'],...
%    ['r=' num2str(round(corr(accuracy_p',participant_scores{:,1}),2))]},'FontSize',28)
text(-0.15,1,'c','Units','normalized','FontSize',24,'FontWeight','bold')
box on
grid on
hold off
%set(ax3,'Position',[0.13,0.0950,0.3347,0.3412])
